...
function ajaxHeart(e) {
  ...
  _axios2.default.post(this.action) // "this" is the event- ie the button that has been clicked in delicious-app
  .then(function (res) {
    var isHearted = _this.heart.classList.toggle('heart__button--hearted'); // this is the form tag, heart is a property of the form tag - the class toggles on click
    (0, _bling.$)('.heart-count').textContent = res.data.hearts.length; // this will make the heart count equal to the length of the heart array
    if (isHearted) {
      _this.heart.classList.add('heart__button--float'); // add classlist of heart button float to give floating heart
      setTimeout(function () {
        return _this.heart.classList.remove('heart__button--float');
      }, 2500); // after 2.5 seconds remove the floating heart class list.
    }
  }).catch(console.error);
};
...
} // similar to fetch (used to get stuff from urls) but with more funcitonality


function typeAhead(search) {
  // takes in the search box - this funtion is used in delicous-app.js
  if (!search) return; // if no search, don't run the funciton
  var searchInput = search.querySelector('input[name="search"]'); // this is where we put our input into the search
  var searchResults = search.querySelector('.search__results');
  searchInput.on('input', function () {
    var _this = this;

    // on is from bling - instead of add event listener- for input
    ...
    _axios2.default // use axios which is similar to fetch
    .get('/api/search?q=' + this.value) // this.value is the input value
    .then(function (res) {
      if (res.data.length) {
        // if the is some data coming back - i.e if the word we put into the search is a word in our database log something to show.
  ...
  searchInput.on('keyup', function (e) {
    // this function is to go up and down on the list produced in search bar
    ...
    var current = search.querySelector('.' + activeClass); // the current is the one with activeClass
    var items = search.querySelectorAll('.search__result'); // grab all the search results and put into items
    var next = void 0;
    if (e.keyCode === 40 && current) {
      // if press down and there is a curent store highlighted
      next = current.nextElementSibling || items[0]; // next will be the next down or the top of list
    } else if (e.keyCode === 40) {
      // if there is none highlight next will equal first in the list
      next = items[0];
...
var heartForms = (0, _bling.$$)('form.heart'); // select all the heart forms
heartForms.on('submit', _heart2.default); // on submit aka click on heart run ajaxHeart
